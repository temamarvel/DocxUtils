...
struct TextOffsetMapper {
    let prefixSums: [Int]

    static func computePrefixSums(for lengths: [Int]) -> [Int] {
    ...
    func locateStart(position: String.Index, in fullText: String) -> TextLocation? {
    ...
    func locateEnd(position: String.Index, in fullText: String) -> TextLocation? {
        let target = fullText.distance(from: fullText.startIndex, to: position)
